...
public class MarkOccurrencesHighlighter implements CaretListener {
        ...
        if (lastRefreshTask == null) {
            lastRefreshTask = rp.create(new Runnable() {
                ...
                public void run() {
                    ...
                    if (selection != null) {
                        Matcher m = getPattern(selection).matcher(comp.getText());
                        while (m.find() == true) {
    ...
    public static Pattern getPattern(String selectedText) {
        // fix regex patterns
        String selection = selectedText.replaceAll("\\(", "\\\\(").replaceAll("\\)", "\\\\)");
        return Pattern.compile(selection);
    }
}